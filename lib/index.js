...
exports.default_abi = new ABI(ffi.FFI_DEFAULT_ABI, 'default_abi');
exports.stdcall_abi = (function (name) {
  if (process.platform === 'win32') {
    if (process.arch === 'x64') {
      return new ABI(ffi.FFI_WIN64, name);
    } else {
      return new ABI(ffi.FFI_STDCALL, name);
    }
  } else {
    return new ABI(ffi.FFI_DEFAULT_ABI, name);
  }
})('stdcall_abi');
exports.winapi_abi = new ABI(+exports.stdcall_abi, 'winapi_abi');
...
// Special C types
exports.void_t    = CType(ref.types.void, 'void');
