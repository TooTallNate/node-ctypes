...
var FunctionPointerType = require('./FunctionPointerType');
...
function FunctionType (abi, returnType, argTypes) {
  var name = returnType.name + '(' + argTypes.map(function (argType) {
    if (!(argType instanceof CType)) {
      throw new Error('not a ctypes type');
    }
    return argType.name;
  }).join(', ') + ')';
...
Object.defineProperty(FunctionType.prototype, 'ptr', {
  enumerable: true,
  configurable: true,
  get: function () {
    return FunctionPointerType(this);
  }
});
